...
#pragma region MemberVariables
	Window32 window = Window32{ L"Vulkan Renderer", 800u, 600u };
...
#pragma endregion
	...
	void mainLoop()
	{
		while (window.ProcessEvents<false>())
		{
			...
			pubTime = std::chrono::duration<float, std::chrono::seconds::period>(currentTime - startTime).count();
		}
...
#pragma region Cleanup
	void cleanupSwapChain()
	{
		...
		for (auto i = 0z; i < swapChainImages.size(); ++i)
		{
	...
	void cleanup()
	{
		...
		for (auto i = 0z; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
...
#pragma endregion
	...
	{
		auto [w, h] = std::tie(window.shape.width, window.shape.height);

		while (w == 0 || h == 0)
		{
			w = window.shape.width;
			h = window.shape.height;
		}
	...
	void createFramebuffers()
	{
		...
		for (auto i = 0z; i < swapChainImageViews.size(); ++i)
		{
	...
	VkImageView createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, std::uint32_t mipLevels)
	{
	...
	void createImage(std::uint32_t width, std::uint32_t height, std::uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
	{
	...
	void generateMipmaps(VkImage image, VkFormat imageFormat, std::int32_t texWidth, std::int32_t texHeight, std::uint32_t mipLevels)
	{
	...
	void transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, std::uint32_t mipLevels)
	{
		VkCommandBuffer commandBuffer = beginSingleTimeCommands();
	...
	void createDescriptorPool()
	{
		...
		poolSizes[0].descriptorCount = static_cast<std::uint32_t>(swapChainImages.size());
		poolSizes[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		poolSizes[1].descriptorCount = static_cast<std::uint32_t>(swapChainImages.size());

		VkDescriptorPoolCreateInfo poolInfo{};
		poolInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
		poolInfo.poolSizeCount = static_cast<std::uint32_t>(poolSizes.size());
		poolInfo.pPoolSizes = poolSizes.data();
		poolInfo.maxSets = static_cast<std::uint32_t>(swapChainImages.size());
	...
	void createDescriptorSets()
	{
		...
		allocInfo.descriptorSetCount = static_cast<std::uint32_t>(swapChainImages.size());
	...
	void createCommandBuffers()
	{
		...
		for (auto i = 0z; i < commandBuffers.size(); ++i)
		{
			VkCommandBufferBeginInfo beginInfo{};
			beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
			...
			vkCmdDrawIndexed(commandBuffers[i], static_cast<std::uint32_t>(indices.size()), 1, 0, 0, 0);
	...
	std::vector<const char*> getRequiredExtensions()
	{
		if constexpr (enableValidationLayers)
		{
			return { VK_EXT_DEBUG_UTILS_EXTENSION_NAME, "VK_KHR_win32_surface", "VK_KHR_surface" };
		}

		else
		{
			return { "VK_KHR_win32_surface", "VK_KHR_surface" };
		}
	}
